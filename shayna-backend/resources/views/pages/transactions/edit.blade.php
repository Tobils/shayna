        ...
        <div class="card-body card-block">
            <form action="{{ route('transactions.update', $item->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="email" name="email" value="{{ old('email') ? old('email') : $item->type }}" class="form-control @error('email') is-invalid @enderror"/>
                    @error('email') <div class="text-muted">{{$message}}</div>@enderror
                </div>

                <div class="form-group">
                    <label for="number" class="form-control-lable">Nomor HP</label>
